...
def format_grammar_string(grammar_dictionary: Dict[str, List[str]]) -> str:
    ...
    return '\n'.join([#f"{nonterminal} = {' / '.join(right_hand_side)}"
                        str(nonterminal) + ' = ' + str(' / '.join(right_hand_side))
...
def format_action(nonterminal: str,
    ...
    if is_string:
        ...
        return str(nonterminal) + ' -> ["\'' + str(right_hand_side) + '\'"]'
    elif is_number:
        #return f'{nonterminal} -> ["{right_hand_side}"]'
        return str(nonterminal) + ' -> ["' + str(right_hand_side) + '"]'
        ...
        return str(nonterminal) + ' -> [' + str(', '.join(child_strings)) + ']'
...
class SqlVisitor(NodeVisitor):
    ...
    def add_action(self, node: Node) -> None:
        ...
        if node.expr.name and node.expr.name not in ['ws', 'wsp']:
            ...
            nonterminal = str(node.expr.name) + ' -> '
            if isinstance(node.expr, Literal):
                #right_hand_side = f'["{node.text}"]'
                right_hand_side = '["' + str(node.text) + '"]'
            else:
                child_strings = []
...
def format_col_unit(col_unit, column_names, table_names):
    ...
    if col_id == 0:
        ...
        result = "%s@%s" % (table_names[column_names[col_id][0]], column_names[col_id][1])
    return agg, result
...
def transform_query_tree(query_tree, schema):
    ...
    for agg_id, val_unit in select_clause[1]:
        ...
        col_unit1[0] = agg_id
        select_columns.append(format_col_unit(col_unit1, column_names, table_names))
    ...
    if groupby_clause:
        for col_unit in groupby_clause:
            groupby_columns.append(format_col_unit(col_unit, column_names, table_names))
    ...
    if orderby_clause:
        ...
        for val_unit in orderby_clause[1]:
            unit_op, col_unit1, col_unit2 = val_unit
            orderby_columns.append(format_col_unit(col_unit1, column_names, table_names))
        ...
        for cond_unit in where_clause:
            ...
            if operator != 'between':
                ...
                if isinstance(val2, dict):
                    ...
                    value2 = 'value'
                where_columns.append((col, operator, str(value1), "and", str(value2)))
    ...
    if having_clause:
        for cond_unit in having_clause:
            ...
            if not_op:
                ...
                operator = WHERE_OPS[op_id]
            agg, col_idx = format_col_unit(col_unit1, column_names, table_names)
            if operator != 'between':
                if isinstance(val1, dict):
                    value1 = '(' + transform_query_tree(val1, schema) + ')'
                else:
                    value1 = 'value'
                having_columns.append((agg + '(%s)' % col_idx, operator, str(value1)))
            else:
                if isinstance(val1, dict):
                    value1 = '(' + transform_query_tree(val1, schema) + ')'
                else:
                ...
                if isinstance(val2, dict):
                    ...
                    value2 = 'value'
                having_columns.append((agg + '(%s)' % col_idx, operator, str(value1), "and", str(value2)))

    sql = "SELECT " + ', '.join([col if agg == '' else agg + '(%s)' % col for agg, col in select_columns])
    ...
    if len(groupby_columns) > 0:
        groupby_str = ' GROUPBY '
        groupby_str += ', '.join([col if agg == '' else agg + '(%s)' % col for agg, col in groupby_columns])
        sql += groupby_str
    ...
    if len(orderby_columns) > 0:
        orderby_str = ' ORDERBY '
        orderby_str += ', '.join([col if agg == '' else agg + '(%s)' % col for agg, col in orderby_columns])
...
def normalize_prolog_variable_names(logical_form):
    ...
    p = logical_form.replace("'", "").lower().replace(" ", "").replace("(", " ( ").replace(")", " ) ").replace(",", " , ")
    cur_vars = []
    toks = p.split(' ')
    new_toks = []
    for w in toks:
