...
class ASTNode:
    def __init__(self, production_rule: ProductionRule, parent, node_id: int, nonterminals_to_skip: list = None):
        ...
        self._rhs_nodes = [None for r in production_rule.rhs_nonterminal if r not in self._nonterminals_to_skip]
    ...
    def add_node(self, node):
        for ridx, r in enumerate(self._rhs_nodes):
            ...
            raise Exception("AST Node %s Already Completed" % str(self._production_rule))
...
class AST:
    def __init__(self, root_rule: ProductionRule, is_sketch_only: bool = False, nonterminals_to_skip: list = None):
        ...
        self._root = ASTNode(root_rule, parent=None, node_id=0, nonterminals_to_skip=nonterminals_to_skip)
    ...
    def add_rule(self, rule):
        node = ASTNode(rule, parent=self._curr_node, node_id=self._curr_node_id, nonterminals_to_skip=self._nonterminals_to_skip)
        self._curr_node_id += 1
